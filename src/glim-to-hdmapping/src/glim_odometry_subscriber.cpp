...
int main(int argc, char **argv)
{
    ...
    while (bag.has_next())
    {
        ...
        if (msg->topic_name == "/glim_ros/points") {
        ...
        if (msg->topic_name == "/glim_ros/odom") {
    ...
    for (int i = 0; i < chunks_pc.size(); i++)
    {
        ...
        fs::path path(outwd);
        std::string filename = ("scan_glim_" + std::to_string(i) + ".laz");
        path /= filename;
        std::cout << "saving to: " << path << " number of points: " << chunks_pc[i].size() << std::endl;
        saveLaz(path.string(), chunks_pc[i]);
        file_names.push_back(filename);

        std::string trajectory_filename = ("trajectory_glim_" + std::to_string(i) + ".csv");
